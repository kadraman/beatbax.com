...
|-----------|---------|---------|
| `chip <name>` | The sound chip that is being emulated. **This is a required directive**. | gameboy |
...
`chip` selects the audio backend for the song. Names, aliases, region qualifiers (`ntsc` / `pal`), and per-chip details are listed under [Sound Chip Plugins](/docs/chips/overview).
...
`scale` alone does not change or filter notes. You opt each channel in with **`lock=...`**, which limits that voice to certain **scale degrees**. Writing (or MIDI-entering) a pitch outside the lock triggers warn/error according to the mode above. In [BeatBax Desktop](/docs/tools/desktop), the same metadata snaps/filters MIDI step entry onto the allowed degrees.
...
You can use `#` for line comments. Everything from `#` to the end of the line is ignored.
